...
// --- INICIALIZAÇÃO DE VARIÁVEIS (BLINDAGEM) ---
$leads_today = 0;
$sales_month = 0.00;
$pipeline_active = 0;
$conversion_rate = 0;
...
try {
    ...
    // Métrica 2: Vendas Confirmadas (Dinheiro)
    // Agora aponta corretamente para a coluna 'value'
    ...
    $stmt->execute($params);
    $sales_month = $stmt->fetchColumn() ?: 0.00;

    // Métrica 3: Em Negociação (Pipeline Ativo)
    // Considera tudo que não é 'New Lead', 'Won' ou 'Lost'
    $sqlPipeline = "SELECT COUNT(*) FROM leads 
                    WHERE funnel_stage NOT IN ('New Lead', 'Won', 'Lost') 
                    $where_user";
    $stmt = $pdo->prepare($sqlPipeline);
    $stmt->execute($params);
    $pipeline_active = $stmt->fetchColumn();

    // Métrica 4: Taxa de Conversão (Won / Total)
    $sqlTotal = "SELECT COUNT(*) FROM leads WHERE 1=1 $where_user";
    $stmt = $pdo->prepare($sqlTotal);
    $stmt->execute($params);
    $total_leads = $stmt->fetchColumn();

    $sqlWonTotal = "SELECT COUNT(*) FROM leads WHERE funnel_stage = 'Won' $where_user";
